...
이름 입력창의 `별풍선 자동 입력`에서 켜기/끄기 토글로 접수한다. 켜면 기존 명단과 이전 접수 내역을 비우고 1구슬당 별풍선 수를 설정할 수 있다. 메시지 대기 시간은 5초로 고정한다. 공식 Chat SDK의 `BALLOON_GIFTED`에는 메시지 본문이 없으므로, SOOP 전자비서와 같이 동일한 userId의 후원 후 첫 채팅을 연결한다. 닉네임을 구슬 이름으로 대신 사용하지 않는다. 후원 후 5초 안에 같은 사람이 작성한 첫 채팅만 연결한다.

후원 건별로 나눈 몫만 반영하고 나머지는 제외한다. 같은 이름은 합산하고 게임의 1,000구슬 한도를 넘는 건은 대기시킨다. 경기 중에는 명단을 수정하지 않고 대기하며 설정 상태로 돌아왔을 때 반영한다. 메시지가 없거나 여러 후원이 연속되어 연결되지 않은 건은 운영자가 이름을 확인한다. 연결 해제/마감 시 미수신 메시지 연결을 중단한다. 내역은 브라우저 메모리에만 두며 새로고침 시 삭제한다. VOD 및 중계방 별풍선은 제외한다.

확인 근거: https://developers.sooplive.com/docs/chatsdk/donation-message 및 SOOP 공식 전자비서 `ServiceNotice-1.0.80.js`의 동일 후원자 채팅 연결 처리. SDK/전자비서 변경 시 다시 검증해야 한다. 모의 이벤트 테스트는 실제 후원 검증을 대신하지 않는다.
