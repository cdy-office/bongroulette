...
이름 입력창의 `별풍선 자동 입력`에서 1구슬당 별풍선 수와 메시지 대기 시간(기본 15초)을 설정하고 접수를 시작한다. 공식 Chat SDK의 `BALLOON_GIFTED`에는 메시지 본문이 없으므로, SOOP 전자비서와 같이 동일한 userId의 후원 후 첫 채팅을 연결한다. 닉네임을 구슬 이름으로 대신 사용하지 않는다. 전자비서의 실제 TTS 대기 시간과 이 설정을 맞춘다.
