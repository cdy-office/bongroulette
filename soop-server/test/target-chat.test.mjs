...
test('targeted speech parses name separately; untargeted speech stays random and ordinary chat stays audience', () => {
  const spoken = [], shown = [];
  const receiver = createChatReceiver({ now: () => 1000, game: () => ({ phase: 'battle', mode: 'arena' }), command: () => false, names: () => ['아무이름123', '새', '새 이름'],
  ...
  send('4', '!새 이름 잘하자'); send('5', '!모두 힘내');
  assert.deepEqual(spoken.slice(-2), [['잘하자', '새 이름'], ['모두 힘내']]);
